...
const handleStartMonthChange = (event) => {
  ...
  if ((currentEnd - selectedStart) / (1000 * 60 * 60 * 24 * 30) > 12) {
    ...
    endMonth.value = `${maxEnd.getFullYear()}-${String(maxEnd.getMonth() + 1).padStart(2, '0')}`;
  }
...
const handleEndMonthChange = (event) => {
  ...
  if ((selectedEnd - currentStart) / (1000 * 60 * 60 * 24 * 30) > 12) {
    ...
    startMonth.value = `${minStart.getFullYear()}-${String(minStart.getMonth() + 1).padStart(2, '0')}`;
  }
...
onMounted(() => {
  ...
  // 設定年月
  startMonth.value = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}`;
  endMonth.value = `${endDate.getFullYear()}-${String(endDate.getMonth() + 1).padStart(2, '0')}`;
