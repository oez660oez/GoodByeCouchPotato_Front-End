...
const formatMoodRecords = (moodRecords) => {
  ...
  return dateRange.map(date => {
    ...
    if (record && record.mood) {
      ...
      console.log(`日期 ${dateString} 的心情值:`, moodValue);
    }
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: function(params) {
        // 檢查是否有數值或數值為 0
        if (params.length === 0 || (!params[0].value && params[0].value !== 0)) {
        ...
        return `${params[0].name} 心情: ${moodValue}`;
      }
    },
    xAxis: {
    type: 'category',
    ...
    yAxis: {
      ...
      axisLabel: {
        formatter: function (value) {
          const labels = ['不透露', '非常不開心', '有點不開心', '普通', '開心', '非常開心'];
          return labels[value]
        }
      }
    },
