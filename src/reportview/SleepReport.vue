...
const API_URLgetsleep = `${Base_URL}/Report/sleep`;
...
const getsleep = async (data) => {
  ...
  if(response.ok){
    ...
    console.log(result);
    const formattedData = formatSleepRecords(result);
...
// 將從後端獲取的數據轉換為圖表需要的格式
const formatSleepRecords = (sleepRecords) => {
  console.log('收到的睡眠記錄:', sleepRecords);
  ...
  return dateRange.map(date => {
    ...
    const record = sleepRecords.find(record => record.hrecordDate === dateString);
    console.log('處理日期:', dateString, '找到的記錄:', record);
    
    let timeValue = 0;
    if (record && record.sleep) {
      const sleepDate = new Date(record.sleep); //new Date(record.sleep) 將字串轉成時間
      const hours = sleepDate.getHours(); //獲得小時
      const minutes = sleepDate.getMinutes(); //獲得分鐘
      timeValue = hours + (minutes / 60); //timevalue=小時.分鐘(/60變成小數)
      console.log(`日期 ${dateString} 的時間值:`, timeValue);
    }
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: function(params) {
        if (!params[0].value && params[0].value !== 0) return `${params[0].name}<br/>無記錄`;
        const hours = Math.floor(params[0].value);
        const minutes = Math.round((params[0].value % 1) * 60);
        const timeString = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`;
        return `${params[0].name}<br/>入睡時間: ${timeString}`;
      }
    },
    xAxis: {
    type: 'category',
    ...
    series: [{
      data: formattedData.map(item => parseFloat(item.time)),
      type: 'line',
      smooth: true,
      lineStyle: {
        width: 3,
